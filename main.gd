...
func _ready():
	track.segments[current_segment_idx].color = Segment.COLOR_ACTIVE


func next_segment():
	var seg =  track.segments[(current_segment_idx + 1) % len(track.segments)]
	if track.segments[current_segment_idx].type_name == Segment.CURVE_DOWN:
		if !do_loop_exit:
...
func _process(_delta):	
	...
	if t>=segment.get_baked_length():
		...
		if segment.type_name == Segment.CURVE_UP: # we're exiting an CURVE_UP segment
		...
		if segment.type_name == Segment.CURVE_DOWN and do_loop_exit:
			speed += 50
		
		if segment.type_name == Segment.CURVE_DOWN and not do_loop_exit: # we're exiting an CURVE_DOWN segment
			# TODO: check if switch track action was pressed			
			# restart on last CURVE_UP segment
			segment.color = Segment.COLOR_IDLE
		...
		segment.color = Segment.COLOR_ACTIVE
		do_loop_exit = false
		track.update()
	...
	if t > segment.get_baked_length() / 2:
		next_segment().color = Segment.COLOR_IDLE
		
		if Input.is_key_pressed(KEY_SPACE):
			do_loop_exit = true
		
		if track.segments[current_segment_idx].type_name == Segment.CURVE_DOWN and next_segment().type_name == Segment.CURVE_UP:
			rider.space_sprite.visible = true
			
			
		next_segment().color = Segment.COLOR_IDLE.linear_interpolate(Segment.COLOR_ACTIVE, t / segment.get_baked_length())
